...
import { fetchAndConvertConnpassEvents, UserProfile, convertConnpassEventToPrismaEvent } from "./utils/connpassEventUtils";
...
async function testConnpassConversion() {
  try {
    ...
    const eventDate = convertedEvent.eventDate ? convertedEvent.eventDate.toISOString().split('T')[0] : '';
