...
export const generateKeywordsAndFetchEvents = async (
  ...
  try {
    ...
    // プロンプトの作成
    const keywordPrompt = `
以下のユーザー情報に基づいて、このユーザーが興味を持ちそうなイベントを検索するための具体的で有用なキーワードを生成してください。
...
【指示】
1. 以下のカテゴリに分類される具体的なキーワードを生成してください：
   - 技術名: 具体的な言語、フレームワーク、ツール名（例：「React」「TensorFlow」「Docker」）
   - イベントタイプ: 具体的なイベント形式（例：「ハッカソン」「ワークショップ」「ハンズオン」）
   - 専門分野: 具体的な技術分野（例：「機械学習」「Web開発」「クラウド」）
   - レベル: 具体的な難易度（例：「初心者向け」「中級者」「上級者」）

2. 以下のような汎用的で検索に役立たないキーワードは避けてください：
   - 避けるべき例: 「プログラミング」「イベント」「技術」「勉強会」「開発」「学習」
   - 代わりに: 「Python」「React」「機械学習」「AWS」のような具体的な技術名や
     「ハンズオン」「LT会」のような具体的なイベント形式を使用

3. ユーザーの興味タグと目標に基づいて、イベント検索に有用なキーワードを30個以上生成してください。
   興味タグの各項目に対して少なくとも1つの具体的なキーワードを含めてください。
