...
イベントテーブルの`location`フィールドが空（null、空文字、「今から」）のレコードに対して、Gemini APIを使用してイベントの詳細情報から場所情報を抽出し、自動的に更新するスクリプトです。

## 機能

- イベントのタイトル、説明、会場、住所情報を解析
- Gemini AIを使用して場所情報を抽出
- 信頼度に基づく更新の可否判定
- レート制限対策（API呼び出し間隔制御）
- ドライランモードでのテスト実行
...
✅ 場所抽出成功: 東京都渋谷区 (信頼度: 0.9)
💾 更新完了: uuid-123 -> 東京都渋谷区

[2/25] 処理中: オンラインVue.js ワークショップ
📍 場所抽出中: オンラインVue.js ワークショップ...
✅ 場所抽出成功: オンライン (信頼度: 0.95)
💾 更新完了: uuid-456 -> オンライン
